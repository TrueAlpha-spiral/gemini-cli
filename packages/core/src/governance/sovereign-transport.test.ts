...
describe('sovereignFetch', () => {
  ...
  it('should return verified response when payload is valid', async () => {
    ...
    (
      verifyKinematicIdentity as unknown as ReturnType<typeof vi.fn>
    ...
    expect(verifyKinematicIdentity).toHaveBeenCalledWith(
      { data: 'valid' },
      'TAS_DNA_CURSIVE_INVARIANT',
    );
  });

  it('should throw PhoenixError when payload fails kinematic verification', async () => {
    // Arrange
    const mockResponse = new Response(JSON.stringify({ drift: 'detected' }), {
      status: 200,
    });
